...
document.addEventListener('DOMContentLoaded', () => {
    ...
    blockReload.addEventListener("click", () => chrome.runtime.sendMessage({ greeting: "blockReload", state: blockReload.checked }) );
    bypassCache.addEventListener("click", () => chrome.runtime.sendMessage({ greeting: "bypassCache", state: bypassCache.checked }) );
...
function toggleReload() {
    ...
    if (interval > 0 && option.checked === true) {
        ...
        chrome.tabs.query({active: true, currentWindow: true, windowType: "normal"}, tab => chrome.runtime.sendMessage({greeting: "deleteReload", tabID: tab[0].id}));
    } else if (interval === 0) {
        option.checked = false;
    }
}
...
function cssTransitionFix() {
    ...
    setTimeout(() => { ref.parentNode.insertBefore(style, ref)}, 50);
}