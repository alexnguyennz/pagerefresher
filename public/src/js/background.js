...
chrome.storage.sync.get({enableContext: false, intervalOne: 5, intervalTwo: 15, intervalThree: 30, intervalFour: 60, intervalFive: 120}, items => {
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    ...
    switch(request.greeting) {
        ...
        case "deleteReload":
            ...
            break;
        case "getTabIDList":
            chrome.tabs.query({active: true, currentWindow: true, windowType: "normal"}, tab => sendResponse({tabIDList: tabIDList, tabID: tab[0].id, tabURL: tab[0].url}));
            break;
        case "blockReload":
            chrome.tabs.query({active: true, currentWindow: true, windowType: "normal"}, tab => blockReload(tab[0].id, request.state));
            break;
        case "bypassCache":
...
function deleteReload(tabID) {
    ...
    if (tabIDList[tabID].hasOwnProperty("reloadInfo") === true) {
        ...
        chrome.browserAction.setBadgeText({tabId: tabID, text: ""}); // reset badge text if reload is turned off
...
function setBadge(tabID) {
    ...
    if (tabIDList[tabID].hasOwnProperty("reloadInfo") === true) {
        ...
        chrome.browserAction.setBadgeBackgroundColor({tabId: tabID, color: "#ff6c52"});
        chrome.browserAction.setBadgeText({tabId: tabID, text: formattedTimeLeft});
    }
...
function blockReload(tabID, state) {
    ...
    (tabIDList[tabID]["blockReload"] === true ? chrome.tabs.executeScript(tabID, { code: "window.stop();" }) : chrome.tabs.reload(tabID));
